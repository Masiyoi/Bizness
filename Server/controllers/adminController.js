...
exports.updateOrderStatus = async (req, res) => {
  try {
    ...
    logActivity({
      ...
      metadata: { order_id: req.params.id, status, tracking_status },
      req,
    }).catch(() => {});
    res.json(result.rows[0]);
  } catch (err) {
...
exports.getActivityLogs = async (req, res) => {
  try {
    const { eventType, userId, from, to, page = 1, limit = 50 } = req.query;
    const conditions = [];
    const params = [];
    if (eventType) { params.push(eventType); conditions.push(`al.event_type = $${params.length}`); }
    ...
    if (to) {
      // Include the entire "to" day — without this, a bare date like
      // "2026-09-17" is parsed as midnight, silently excluding anything
      // logged later that same day.
