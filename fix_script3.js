...
if (!patch(MEMBERS_PATH, oldGetProfile, newGetProfile, "membersController.js: getProfile returns referral_code")) {
  // Fallback: try again with an extra blank line after the 404 check,
  // matching what the diff script found in the actual file.
  const oldWithBlank = oldGetProfile.replace(
    "if (!member) return res.status(404).json({ error: 'Not a member' });\n\n    const { rows: activities }",
    "if (!member) return res.status(404).json({ error: 'Not a member' });\n\n\n    const { rows: activities }"
  );
  patch(MEMBERS_PATH, oldWithBlank, newGetProfile, "membersController.js: getProfile returns referral_code (retry w/ extra blank line)");
}
console.log("\nDone.");
