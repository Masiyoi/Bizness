...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0 }
  ...
  .lp-hero-panel + .lp-hero-panel { border-left: none; }
  ...
  .lp-search-wrap { padding: 16px clamp(20px,5%,80px); display: flex; align-items: center; gap: 12px; }
  ...
  .lp-editorial { display: grid; grid-template-columns: 1fr 1fr; min-height: 70vh; overflow: hidden; }
  ...
  .lp-editorial-img img { width: 100%; height: 100%; min-height: 320px; object-fit: cover; filter: contrast(1.05); opacity: 1; transition: transform 6s ease, opacity 0.4s; }
  .lp-editorial-img:hover img { transform: scale(1.05); opacity: 0.95 }
  .lp-editorial-badge { position: absolute; top: 20px; left: 20px; font-family: var(--f-sans); font-size: 9px; font-weight: 700; letter-spacing: 3px; text-transform: uppercase; color: #fff; text-shadow: 0 1px 8px rgba(0,0,0,0.5); }
  .lp-editorial-copy { display: flex; flex-direction: column; justify-content: flex-end; padding: clamp(32px,5vw,72px); background: #fff; }
  ...
  .lp-editorial-copy p { font-family: var(--f-sans); font-size: clamp(13px,1.4vw,15px); font-weight: 300; color: var(--ink); line-height: 1.8; margin-bottom: 28px; max-width: 340px; }
  ...
  .lp-chatbot-fab.open .lp-chatbot-main { width: auto; height: auto; cursor: pointer; touch-action: auto; }
  .lp-chatbot-fab-img { width: 100%; height: 100%; border-radius: 50%; object-fit: cover; display: block; flex-shrink: 0; box-shadow: 0 4px 16px rgba(0,0,0,0.28); transition: width 0.35s, height 0.35s; }
  .lp-chatbot-fab.open .lp-chatbot-fab-img { width: 44px; height: 44px; }
  .lp-chatbot-fab-badge { position: absolute; top: -2px; right: -2px; width: 20px; height: 20px; border-radius: 50%; background: #0A0A0A; color: #fff; font-family: var(--f-sans); font-size: 14px; font-weight: 700; line-height: 1; display: flex; align-items: center; justify-content: center; box-shadow: 0 2px 6px rgba(0,0,0,0.3); pointer-events: none; }
  ...
  return (
    <section className="lp-hero-split">
      ...
      <div className="lp-hero-panel lp-fade-up lp-d2">
        <img src="/i2.webp" alt="PW Essentials" className="lp-hero-panel-img" />
        <div className="lp-hero-panel-overlay" />
        <div className="lp-hero-panel-content">
  ...
  return (
    <section className="lp-bestsellers">
      ...
      <div className="lp-bestsellers-content">
        <h2 className="lp-bestsellers-title">Best Sellers</h2>
        <p className="lp-bestsellers-desc">
          Handpicked by you, loved by thousands. These are the undisputed non-negotiables everyone is reaching out for right now.
        </p>
        <button className="lp-hero-cta-primary" onClick={() => navigate('/categories/best-sellers')}>
          Community Favorites
        </button>
      </div>
    </section>
...
export default function Homepage() {
 ...
 const categoryBannerItems: { slug: string; name: string }[] = [
  ...
  const activeBaseSlug = activeCategory === 'all' ? null : activeCategory.slice(genderFilter.length + 1);

  let filtered = products
    .filter(p => !(p.id in flashSaleMap))
    .filter(p =>
      p.category_gender === genderFilter &&
      (activeBaseSlug === null || p.category_slug === activeBaseSlug) &&
