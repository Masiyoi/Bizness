...
export default function ProductDetail() {
  ...
  // Sync cart state — also pre-fill selected color/size from existing cart item
  useEffect(() => {
    ...
    axios.get('/api/cart', { headers: { Authorization: `Bearer ${token}` } })
      .then(res => {
        ...
        if (cartItem) {
          setInCart(true);
          // ✅ Pre-fill selectors with whatever is already saved in the cart
  ...
  const handleCartToggle = async () => {
    ...
    try {
      if (inCart) {
        // Find the cart item id so we can delete by item id (not product id)
        const cartRes = await axios.get('/api/cart', authHeaders());
        const cartItem = cartRes.data.find((i: any) => i.product_id === product!.id);
        if (cartItem) {
          await axios.delete(`/api/cart/${cartItem.id}`, authHeaders());
        }
        setInCart(false);
        setSelectedColor('');
        setSelectedSize('');
        ...
        // ✅ Use selected_color and selected_size — matching backend field names exactly
        await axios.post(
          '/api/cart',
          {
  ...
  // When user changes color/size while item is already in cart — update DB immediately
    ...
    try {
      ...
      if (cartItem) {
        await axios.patch(
          ...
          { quantity: cartItem.quantity, selected_color: color, selected_size: selectedSize || cartItem.selected_size },
          authHeaders()
        );
        window.dispatchEvent(new CustomEvent('cartUpdated'));
  ...
  const handleSizeChange = async (size: string) => {
    ...
    try {
      ...
      if (cartItem) {
        await axios.patch(
          `/api/cart/${cartItem.id}`,
          { quantity: cartItem.quantity, selected_color: selectedColor || cartItem.selected_color, selected_size: size },
          authHeaders()
        );
        window.dispatchEvent(new CustomEvent('cartUpdated'));
          ...
          <div>
              ...
              <div style={{ marginBottom:22 }}>
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:12 }}>
                  ...
                  {selectedColor && (
                    <span style={{ fontFamily:"'Jost',sans-serif", fontSize:12, fontWeight:700, color:T.navy }}>
                      {selectedColor}
                      {/* ✅ Show "saved" badge when item is already in cart */}
                  ...
                  {product.colors.map((color, i) => {
                    ...
                    return (
                      <button
                        key={i}
                        title={color}
                        onClick={() => handleColorChange(color)}  // ✅ use handler (not setter) so cart updates too
                        style={{
                ...
                <div className={colorError ? 'lp-shake' : ''}>
                  <div style={{ position:'relative' as const }}>
                    ...
                    <select
                      value={selectedColor}
                      onChange={e => handleColorChange(e.target.value)}  // ✅ use handler
                      style={{
                        width: '100%',
                        background: T.cream,
                    ...
                    <div style={{
                      ...
                      pointerEvents:'none', fontSize:10, color:T.muted, lineHeight:1,
                    }}>▼</div>
                  </div>
                  ...
                  {colorError ? (
                    ...
                    <div style={{ fontFamily:"'Jost',sans-serif", fontSize:11, color:T.muted, marginTop:6 }}>
                      {product.colors.length} colour{product.colors.length !== 1 ? 's' : ''} available — pick one above
                    </div>
                  )}
                </div>
              </div>
            ...
            {hasSizes && (
              <div style={{ marginBottom: 22 }}>
                ...
                <div className={sizeError ? 'lp-shake' : ''} style={{ display:'flex', flexWrap:'wrap', gap:8, marginBottom:10 }}>
                  {product.sizes.map((size, i) => {
                    ...
                    return (
                      <button
                        key={i}
                        onClick={() => handleSizeChange(size)}  // ✅ use handler
                        style={{
                          minWidth: 44, padding: '8px 14px',
                          borderRadius: 8,
                ...
                {sizeError ? (
                  <div style={{ fontFamily:"'Jost',sans-serif", fontSize:11, color:'#C0392B', marginTop:4, fontWeight:700 }}>
                    ⚠ Please select a size to continue
                  </div>
                ) : selectedSize ? (
                  <div style={{ fontFamily:"'Jost',sans-serif", fontSize:11, color:'#4A7A4A', marginTop:4, fontWeight:600 }}>
