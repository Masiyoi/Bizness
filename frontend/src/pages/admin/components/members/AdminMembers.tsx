...
function AdminUpdatesManager() {
  ...
  const load = () => {
    setLoading(true);
    axios.get('/api/members/admin/updates').then(r => setItems(r.data)).finally(() => setLoading(false));
  };
  useEffect(() => { load(); }, []);
  const handleUpload = async () => {
    ...
    try {
      await axios.post('/api/members/admin/updates', fd, { headers: { 'Content-Type': 'multipart/form-data' } });
  ...
  const toggleActive = (item: any) =>
    axios.put(`/api/members/admin/updates/${item.id}`, { is_active: !item.is_active }).then(load);
  const remove = (id: number) =>
    axios.delete(`/api/members/admin/updates/${id}`).then(load);
  const move = (index: number, dir: -1 | 1) => {
    ...
    setItems(newOrder);
    axios.put('/api/members/admin/updates/reorder', { order: newOrder.map(i => i.id) });
  };
  return (
    <div className="pf-card" style={{ padding: 20, marginBottom: 24 }}>
      <h2 style={{ fontFamily: "'Cormorant Garamond',serif", fontWeight: 700, fontSize: 22, marginBottom: 16 }}>
        Members-Only Carousel
      </h2>
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center', marginBottom: 20 }}>
        ...
        <button onClick={handleUpload} disabled={uploading || !file || !title}
          style={{ padding: '9px 16px', background: '#111', color: '#fff', border: 'none', cursor: 'pointer' }}>
      ...
      {loading ? <p>Loading…</p> : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {items.map((item, i) => (
            <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: 12, border: '1px solid #eee', padding: 10 }}>
              {item.type === 'image'
                ? <img src={item.media_url} style={{ width: 64, height: 64, objectFit: 'cover' }} />
                : <video src={item.media_url} style={{ width: 64, height: 64, objectFit: 'cover' }} muted />}
              <div style={{ flex: 1 }}>
                <p style={{ fontWeight: 700 }}>{item.title}</p>
                <p style={{ fontSize: 12, color: '#888' }}>{item.subtitle}</p>
              </div>
              <span style={{ fontSize: 11, color: item.is_active ? '#3A7A3A' : '#999' }}>
                {item.is_active ? 'Live' : 'Hidden'}
              </span>
              <button onClick={() => move(i, -1)}>↑</button>
              <button onClick={() => move(i, 1)}>↓</button>
              <button onClick={() => toggleActive(item)}>{item.is_active ? 'Hide' : 'Show'}</button>
              <button onClick={() => remove(item.id)} style={{ color: '#C0392B' }}>Delete</button>
            </div>
          ))}
        </div>
      )}
