...
const userIcon = '/orders/user.png';
const billIcon = '/orders/bill.png';
const mapIcon  = '/orders/map.png';
const TRACKING_ICON: Record<string, string> = {
  'Delivered':            '/delivered.png',
  'Delivery in progress': '/deliveryinprogress.png',
};
      ...
      <div style={{ background: T.white, borderRadius: 18, width: '100%', maxWidth: 620, maxHeight: '94vh', overflowY: 'auto', boxShadow: '0 32px 80px rgba(0,0,0,0.25)', animation: 'wizardIn 0.28s cubic-bezier(.34,1.56,.64,1)' }}>
        ...
        <div style={{ background: T.black, borderRadius: '18px 18px 0 0', padding: '20px 26px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            }}><img src={TRACKING_ICON[order.tracking_status] || deliveryIcon} alt="" style={{ width: 12, height: 12, verticalAlign: 'middle', marginRight: 4 }} />{order.tracking_status}</span>
        ...
        <div style={{ padding: '22px 26px', display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            ...
            <div style={{ padding: '4px 0' }}>
              ...
              {order.mpesa_receipt && (
                ...
                ><img src={billIcon} alt="" style={{ width: 12, height: 12, verticalAlign: 'middle', marginRight: 5 }} />{order.mpesa_receipt} <span style={{ fontSize: 9, opacity: 0.6 }}>copy</span></button>
              )}
              <div style={{ fontFamily: 'Jost,sans-serif', fontSize: 11, color: T.grey1 }}><img src={TRACKING_ICON[order.tracking_status] || deliveryIcon} alt="" style={{ width: 12, height: 12, verticalAlign: 'middle', marginRight: 4 }} />{order.tracking_status}</div>
            </div>
          </div>
          ...
          {(shippingInfo || zoneLabel) && (
            <div style={{ background: T.white, border: `1px solid ${T.grey3}`, borderRadius: 12, overflow: 'hidden' }}>
              <div style={{ background: T.black, padding: '10px 16px', display: 'flex', alignItems: 'center', gap: 9 }}>
                <img src={mapIcon} alt="" style={{ width: 14, height: 14, flexShrink: 0 }} />
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                    ...
                    {(shippingInfo.county || shippingInfo.town) && (
                      <div>
                        ...
                        <div style={{ fontFamily: 'Jost,sans-serif', fontSize: 13, color: T.black }}><img src={mapIcon} alt="" style={{ width: 11, height: 11, verticalAlign: 'middle', marginRight: 5 }} />{[shippingInfo.town, shippingInfo.county].filter(Boolean).join(', ')}</div>
                      </div>
                    )}
                    ...
                    {shippingInfo.additionalInfo && (
                      <div style={{ gridColumn: '1/-1', padding: '10px 0', borderTop: `1px solid ${T.grey3}` }}>
          ...
          {/* ── Items ── */}
          <div style={{ background: T.white, border: `1px solid ${T.grey3}`, borderRadius: 12, overflow: 'hidden' }}>
            <div style={{ borderBottom: `1px solid ${T.grey3}`, padding: '10px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            {items.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ borderTop: `2px solid ${T.grey3}`, padding: '12px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
