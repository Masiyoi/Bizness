...
function VariantGrid({ colors, sizes, variants, onChange }: {
  ...
  const variantExists = variants.some(v => variantKey(v.color, v.size) === key);

  if (variantExists) {
    // If it exists, update it via map
    onChange(variants.map(v => variantKey(v.color, v.size) === key ? { ...v, stock } : v));
  } else {
    // If it was previously filtered out, append it to the matrix
    onChange([...variants, { color, size, stock, sku: '' }]);
  }
};
...
export function AddProductWizard({ onClose, onSaved, editProduct }: WizardProps) {
  ...
  const [department,  setDepartment]  = useState<'footwear' | 'clothing'>('footwear');
  const [categoryId,  setCategoryId]  = useState<string>(editProduct?.category_id || '');
  ...
  useEffect(() => {
    axios.get('/api/categories/tree').then(res => setTree(res.data)).catch(() => {});
  }, []);
  useEffect(() => {
    const cid = editProduct?.category_id;
    if (!tree || !cid) return;
    (['men', 'women'] as const).forEach(g => {
      (['footwear', 'clothing'] as const).forEach(d => {
        if (tree[g][d].some(c => c.id === cid)) {
          setGender(g);
          setDepartment(d);
          setCategoryId(cid);
        }
  ...
  if (!editProduct?.variants?.length) return [];
            ...
            <div style={{ animation: 'fadeUp 0.25s ease both', display: 'flex', flexDirection: 'column', gap: 20 }}>
              ...
              <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 7 }}>
                  <select value={gender} onChange={e => { const g = e.target.value as 'men' | 'women'; setGender(g); setCategoryId(''); }} style={inp}>
                    <option value="men">Men</option>
                    <option value="women">Women</option>
                  </select>
                  <select value={department} onChange={e => { const d = e.target.value as 'footwear' | 'clothing'; setDepartment(d); setCategoryId(''); }} style={inp}>
                    <option value="footwear">Footwear</option>
                    <option value="clothing">Clothing</option>
                  </select>
                  <select value={categoryId} onChange={e => setCategoryId(e.target.value)} style={inp}>
                    <option value="">Select subcategory</option>
                    {(tree?.[gender]?.[department] ?? []).map(c => (
            ...
            <div style={{ animation: 'fadeUp 0.25s ease both' }}>
              ...
              <div style={{ border: `1px solid ${T.grey3}`, borderRadius: 14, overflow: 'hidden', marginBottom: 20, background: T.white }}>
                <div style={{ position: 'relative', height: 180, background: T.grey4, overflow: 'hidden' }}>
                  ...
                  {(() => {
                    const selectedCategoryName = tree?.[gender]?.[department]?.find(c => c.id === categoryId)?.name || '';
                    return selectedCategoryName && (
                      <div style={{ position: 'absolute', top: 10, left: 10, background: 'rgba(0,0,0,0.75)', color: T.white, borderRadius: 4, padding: '3px 9px', fontFamily: 'Jost,sans-serif', fontSize: 9, fontWeight: 700, letterSpacing: '1.5px', textTransform: 'uppercase' }}>{selectedCategoryName}</div>
                    );
                  })()}
                </div>
