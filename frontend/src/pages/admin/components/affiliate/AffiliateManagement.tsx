...
export function AffiliateManagement({ showToast }: AffiliateManagementProps) {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await axios.patch(`/api/affiliate/salespersons/${deleteTarget.id}`, { status: 'inactive' });

      showToast(`✓ Removed ${deleteTarget.name} — their code no longer applies`);
      if (expandedId === deleteTarget.id) setExpandedId(null);
      loadSalespersons();
    } catch (err) {
      console.error(err);
      showToast('Failed to remove salesperson', 'err');
    } finally {
      setDeleting(false);
    ...
    <div style={{ fontFamily: "'Jost',sans-serif" }} className="fade-up">
      ...
      {deleteTarget && (
        <ConfirmDialog
          ...
          message={`Remove ${deleteTarget.name}? Their coupon code "${salespersons.find(s => s.id === deleteTarget.id)?.coupon_code || ''}" will stop working immediately. Their order and commission history is kept.`}
