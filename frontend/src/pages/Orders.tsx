...
export default function Orders() {
  ...
  <img src="/noorders.png" alt="No orders" style={{ width:'56%', height:'56%', objectFit:'contain' }} />
        ...
        {orders.map((order, idx) => {
          ...
          const trackingIdx     = TRACKING_STEPS.findIndex(s => s.label.trim().toLowerCase() === (order.tracking_status || '').trim().toLowerCase());
              ...
              <div className="order-header" onClick={() => toggleExpand(order.id)}>  {/* uses order.id */}
                <div className="order-tile-row">
                  <div className="tile-left-col">
                    ...
                    <div className="tile-value" style={{ fontWeight:600, fontSize:'clamp(13px,3vw,15px)', color:T.muted, letterSpacing:'0.3px' }}>KSh {Number(order.total_amount).toLocaleString()}</div>
                  </div>
