...
interface Order {
  ...
  total_amount: string;
  delivery_fee?: string;        // FIX #4: marked optional — can be absent from API response
  delivery_zone?: string;       // FIX #4: marked optional — can be absent from API response
...
// ── Review statuses per product — tracks if user already reviewed
type ReviewMap = Record<number, boolean>; // product_id → reviewed?
      ...
      <div style={{ background:T.white, borderRadius:20, width:'100%', maxWidth:460, boxShadow:'0 32px 80px rgba(13,27,62,0.3)', overflow:'hidden', animation:'modalIn 0.22s ease' }}>
        ...
        <div style={{ background:T.navy, padding:'18px 22px', display:'flex', alignItems:'center', gap:12 }}>
          ...
          <div style={{ flex:1, minWidth:0 }}>
            ...
            {item.category && <div className="jost" style={{ fontSize:10, color:`rgba(200,169,81,0.7)`, letterSpacing:'1.5px', textTransform:'uppercase', marginTop:3 }}>{item.category}</div>}
          </div>
          <button onClick={onClose} style={{ background:'rgba(255,255,255,0.08)', border:'none', color:'rgba(255,255,255,0.6)', width:28, height:28, borderRadius:7, cursor:'pointer', fontSize:13, display:'flex', alignItems:'center', justifyContent:'center' }}>✕</button>
        </div>
        {/* Body */}
        <div style={{ padding:22 }}>
            ...
            <div style={{ display:'flex', gap:4 }}>
              {[1,2,3,4,5].map(s => (
                <span key={s}
                  ...
                  style={{ fontSize:30, cursor:'pointer', color: s<=(hover||rating)?T.gold:T.creamDeep, transition:'all 0.15s', transform: s<=(hover||rating)?'scale(1.2)':'scale(1)', display:'inline-block', userSelect:'none' }}>★</span>
              ))}
            </div>
            <div className="jost" style={{ fontSize:11, color:T.gold, marginTop:6, fontWeight:600, minHeight:18 }}>
          ...
          <div style={{ marginBottom:18 }}>
            ...
            <div className="jost" style={{ fontSize:10, color:T.muted, textAlign:'right', marginTop:3 }}>{comment.length}/1000</div>
          </div>
          {error && <div className="jost" style={{ fontSize:12, color:'#C0392B', background:'#FDF0EE', border:'1px solid #F5C6C0', borderRadius:8, padding:'8px 12px', marginBottom:14 }}>{error}</div>}
          <div style={{ display:'flex', gap:10 }}>
            ...
            <button onClick={handleSubmit} disabled={saving} className="jost"
...
export default function Orders() {
  ...
  // FIX #1 & FIX #3: Moved fetchReviewedStatus ABOVE fetchOrders so it is declared
  // before it is called. Also removed the unused `orderList` parameter.
  // const arrow functions are NOT hoisted — calling one before its declaration
  // throws a ReferenceError at runtime.
  const fetchReviewedStatus = useCallback(async () => {   // FIX #1, #3
    try {
      const res = await axios.get('/api/reviews/my', authHeaders());
      const reviewed: ReviewMap = {};
      for (const r of res.data) {
        // FIX #2: Explicitly coerce to Number so the key type is always consistent
        // with how we look it up (item.product_id is a number).
        reviewed[Number(r.product_id)] = true;
      }
      setReviewedMap(reviewed);
    } catch { /* ignore — non-critical */ }
  }, []);

  // FIX #6: Wrap in useCallback so the useEffect dependency array is stable
  ...
  const fetchOrders = useCallback(async () => {
    try {
      ...
      setOrders(sorted);
      // FIX #1 & #3: fetchReviewedStatus is now declared above, so this call is safe.
      // The unused `sorted` argument has also been removed from the call site.
      fetchReviewedStatus();
    } catch (err: any) {
  ...
  }, [navigate, fetchReviewedStatus]); // FIX #6: stable deps

  // FIX #5: Added `navigate` and `fetchOrders` to the dependency array to
  // avoid a stale-closure warning and potential bugs in React Strict Mode.
  useEffect(() => {
    if (!token) { navigate('/login'); return; }
    fetchOrders();
  }, [navigate, fetchOrders, token]); // FIX #5
  ...
  const handleReviewSubmit = async (productId: number, rating: number, comment: string) => {
    ...
    // FIX #2: Coerce to Number for consistent key lookup
    setReviewedMap(prev => ({ ...prev, [Number(productId)]: true }));
    showToast('✓ Review submitted — thank you!');
  };

  // Orders eligible for reviews: confirmed, shipped, or delivered (not cancelled)
    ...
    <div style={{ fontFamily: "'Playfair Display','Georgia',serif", background: T.cream, minHeight: '100vh', color: T.text, overflowX: 'hidden' }}>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .skel{background:linear-gradient(90deg,${T.creamMid} 25%,${T.cream} 50%,${T.creamMid} 75%);background-size:200% 100%;animation:sk 1.4s infinite;border-radius:6px}
      ...
      {/* Review Modal */}
      {reviewModal && (
        <ReviewModal item={reviewModal} onClose={() => setReviewModal(null)} onSubmit={handleReviewSubmit}/>
      )}
      ...
      {/* Navbar */}
      <nav style={{ background: T.navy, padding: '0 clamp(16px,5%,5%)', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 100, boxShadow: `0 4px 32px rgba(13,27,62,0.35)`, borderBottom: `1px solid rgba(200,169,81,0.25)` }}>
        <button className="back-btn" onClick={() => navigate('/')}>← <span>Shop</span></button>
      ...
      <div style={{ padding: `clamp(24px,5vw,40px) clamp(14px,5%,5%) clamp(60px,10vw,80px)`, maxWidth: 860, margin: '0 auto' }}>
        {error && (
          <div className="jost" style={{ background: '#FDF0EE', border: '1px solid #F5C6C0', borderRadius: 10, padding: '12px 18px', color: '#C0392B', fontSize: 13, marginBottom: 24 }}>{error}</div>
        )}
        ...
        {orders.length === 0 && !loading && (
          <div className="fade-in" style={{ textAlign: 'center', padding: '60px 0' }}>
            ...
            <p className="jost" style={{ fontSize: 14, color: T.muted, marginBottom: 32, lineHeight: 1.7, fontWeight: 300 }}>You haven't placed any orders yet.<br/>Explore our premium collection.</p>
        ...
        {orders.map((order, idx) => {
          ...
          // FIX #4: delivery_fee is now optional on the interface, so guard with ?? '0'
          const subtotal    = Number(order.total_amount) - Number(order.delivery_fee ?? '0');
          // FIX #4: delivery_zone is now optional on the interface
          const zoneLabel   = DELIVERY_ZONE_LABELS[order.delivery_zone ?? ''] ?? order.delivery_zone ?? '—';
          const totalQty    = order.items.reduce((s, i) => s + i.quantity, 0);

          const TRACKING_STEPS = [
            { icon:'✅', label:'Order Placed' }, { icon:'💳', label:'Payment Confirmed' },
            { icon:'🔄', label:'Processing'   }, { icon:'📦', label:'Packed'            },
            { icon:'🚚', label:'Shipped'      }, { icon:'🛵', label:'Out for Delivery'  },
          ...
          const currentTrackIdx = trackingIdx !== -1 ? trackingIdx :
            order.status === 'pending'    ? 0 : order.status === 'confirmed'  ? 1 :
            order.status === 'processing' ? 2 : order.status === 'shipped'    ? 4 :
              ...
              <div className="order-header" onClick={() => toggleExpand(order.id)}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div className="order-title-row">
                    <span style={{ fontFamily: "'Playfair Display',serif", fontWeight: 700, fontSize: 'clamp(14px,3.5vw,16px)', color: T.navy }}>Order #{order.id}</span>
                ...
                <div style={{ textAlign: 'right', flexShrink: 0, minWidth: 72 }}>
                  <div className="jost" style={{ fontWeight: 800, fontSize: 'clamp(14px,3.5vw,17px)', color: T.navy }}>KSh {Number(order.total_amount).toLocaleString()}</div>
                    ...
                    <div style={{ padding: 'clamp(16px,4vw,20px) clamp(14px,4vw,24px)', borderBottom: `1px solid ${T.creamDeep}` }}>
                      ...
                      <div style={{ display:'flex', alignItems:'flex-start', gap:0 }}>
                        {TRACKING_STEPS.map((s, i) => {
                          const isPast = i < currentTrackIdx, isCurrent = i === currentTrackIdx;
                          return (
                            <div key={s.label} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', position:'relative' }}>
                              ...
                              <div className="track-circle" style={{ width:26, height:26, borderRadius:'50%', zIndex:1, background:isPast?T.gold:isCurrent?T.navy:'#fff', border:isCurrent||isPast?`2px solid ${T.gold}`:`2px solid ${T.creamDeep}`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:isPast?10:isCurrent?12:9, boxShadow:isCurrent?`0 0 0 4px rgba(200,169,81,0.2)`:'none', transition:'all 0.4s', flexShrink:0 }}>
                                {isPast ? <span style={{ color:T.navy }}>✓</span> : isCurrent ? <span>{s.icon}</span> : <span style={{ color:T.muted, fontFamily:"'Jost',sans-serif", fontWeight:700 }}>{i+1}</span>}
                              </div>
                              <div className="jost track-label" style={{ fontSize:9, fontWeight:isCurrent?700:500, color:isCurrent?T.navy:isPast?T.gold:T.muted, marginTop:6, letterSpacing:'0.3px', textAlign:'center', lineHeight:1.3, opacity:isPast||isCurrent||i===currentTrackIdx+1?1:0.45 }}>{s.label}</div>
                            </div>
                          );
                        })}
                      </div>
                  ...
                  <div style={{ padding: `clamp(16px,4vw,20px) clamp(14px,4vw,24px)` }}>
                    ...
                    <div>
                      {order.items.map(item => {
                        // FIX #2: Coerce to Number for consistent key lookup in reviewedMap
                        ...
                        return (
                          <div key={item.id} className="item-row">
                            ...
                            <div style={{ flex:1, minWidth:0 }}>
                              {item.category && (
                                <div className="jost" style={{ display:'inline-block', background:T.navy, color:T.gold, borderRadius:3, padding:'1px 7px', fontSize:9, fontWeight:700, letterSpacing:'1.5px', textTransform:'uppercase', marginBottom:4 }}>{item.category}</div>
                              )}
                              ...
                              <div className="jost" style={{ fontSize:11, color:T.muted, marginTop:2 }}>Qty: {item.quantity} · KSh {Number(item.price).toLocaleString()} each</div>
                            </div>
                            <div style={{ display:'flex', flexDirection:'column', alignItems:'flex-end', gap:6, flexShrink:0 }}>
                              ...
                              {eligible && (
                                reviewed ? (
                                  ...
                                  <button className="review-btn" onClick={() => setReviewModal(item)}>
                                    ★ Review
                                  </button>
                                )
                              )}
                    ...
                    <div className="summary-grid">
                      <div style={{ background:T.cream, border:`1px solid ${T.creamDeep}`, borderRadius:14, padding:'clamp(12px,3vw,16px) clamp(12px,3vw,18px)' }}>
                        ...
                        <div style={{ display:'flex', justifyContent:'space-between', marginBottom:8, gap:8 }}>
                          ...
                          <span className="jost" style={{ fontSize:12, fontWeight:600, color:Number(order.delivery_fee ?? '0')===0?'#4A7A4A':T.navy, flexShrink:0 }}>
      ...
      <footer style={{ background: T.navy, borderTop: `1px solid rgba(200,169,81,0.2)` }}>
        ...
        <div style={{ padding: `20px clamp(16px,5%,5%)`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
          <div style={{ fontFamily: "'Playfair Display',serif", fontWeight: 700, fontSize: 18, color: T.white }}>Luku <span style={{ color: T.gold }}>Prime</span></div>
          <div className="jost" style={{ fontSize: 11, color: 'rgba(255,255,255,0.25)' }}>© 2025 Luku Prime · All rights reserved</div>
          <div style={{ display: 'flex', gap: 20 }}>
            {['Privacy','Terms','Help'].map(l => (
              <span key={l} className="jost" style={{ cursor:'pointer', color:'rgba(255,255,255,0.35)', fontSize:12, letterSpacing:'1px', textTransform:'uppercase', transition:'color 0.2s', minHeight:44, display:'flex', alignItems:'center' }}
