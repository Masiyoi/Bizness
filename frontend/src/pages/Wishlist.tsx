...
const fetchCart = useCallback(async () => {
  ...
  return (
    <div style={{ fontFamily:"'Playfair Display','Georgia',serif", background:T.cream, minHeight:'100vh', color:T.text, overflowX:'hidden' }}>
      ...
      {/* ── HERO HEADER ── */}
      <div style={{ background:'#FFFFFF', position:'relative', overflow:'hidden', padding:'clamp(40px,6vw,72px) 5% clamp(36px,5vw,60px)' }}>
        <div style={{ position:'absolute', top:'-20%', right:'-5%', width:500, height:500, borderRadius:'50%', background:'transparent', pointerEvents:'none' }} />
        <div style={{ position:'absolute', bottom:'-30%', left:'10%', width:300, height:300, borderRadius:'50%', background:'transparent', pointerEvents:'none' }} />

        <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:24 }}>
          <span className="jost" style={{ fontSize:11, color:'#000000', letterSpacing:'2px', textTransform:'uppercase', cursor:'pointer', fontWeight:700 }} onClick={() => navigate('/')}>Home</span>
          <span style={{ color:'rgba(0,0,0,0.4)', fontSize:10 }}>›</span>
          <span className="jost" style={{ fontSize:11, color:'#000000', letterSpacing:'2px', textTransform:'uppercase', fontWeight:700 }}>Wishlist</span>
        </div>
      ...
      <div style={{ height:2, background:'linear-gradient(90deg,transparent 0%,rgba(0,0,0,0.1) 30%,rgba(0,0,0,0.18) 50%,rgba(0,0,0,0.1) 70%,transparent 100%)' }} />
              ...
              <div style={{ display:'flex', flexWrap:'wrap', gap:10, justifyContent:'center' }}>
                {['Clothes', 'Shoes', 'Bags', 'Jackets', 'Sneakers', 'Socks'].map(cat => (
                  <button key={cat} className="jost" onClick={() => navigate('/')}
                    style={{ background:'#fff', border:'1px solid #E0E0E0', borderRadius:50, padding:'8px 18px', fontSize:12, fontWeight:500, color:'#333333', cursor:'pointer', transition:'all 0.2s' }}
                    onMouseEnter={e => { e.currentTarget.style.borderColor='#000000'; e.currentTarget.style.background='#F5F5F5'; }}
                  ...
                  <div key={item.product_id} className={`wl-card ${leaving ? 'leaving' : 'entering'}`}>
                    ...
                    <div style={{ padding:'14px 16px 16px', flex:1, display:'flex', flexDirection:'column' }}>
                      ...
                      <div style={{ marginBottom:10 }}>
                        {stock === 0
                          ? <span className="jost" style={{ fontSize:9, fontWeight:700, color:'#333333', background:'#F0F0F0', border:'1px solid #CCCCCC', borderRadius:3, padding:'2px 7px' }}>Out of stock</span>
                          : stock <= 5
                          ? <span className="jost" style={{ fontSize:9, fontWeight:700, color:'#333333', background:'#F5F5F5', border:'1px solid #CCCCCC', borderRadius:3, padding:'2px 7px' }}>⚠ Only {stock} left</span>
                          : <span className="jost" style={{ fontSize:9, fontWeight:600, color:'#000000', background:'#F0F0F0', border:'1px solid #CCCCCC', borderRadius:3, padding:'2px 7px' }}>✓ In Stock</span>
                        }
                      </div>
                      <div className="jost" style={{ fontWeight:800, fontSize:17, color:'#1a1a1a', marginBottom:14 }}>
                        KSh {Number(item.price).toLocaleString()}
                      </div>
                      <div style={{ display:'flex', gap:8, marginTop:'auto' }}>
                        <button className="jost wl-cart-btn" disabled={stock === 0 || busy}
                          onClick={() => stock > 0 && toggleCart(item.product_id)}
                          style={{ flex:1, background:stock===0?'#F0F0F0':inCart?'#EEEEEE':'#1a1a1a', color:stock===0?'#AAAAAA':inCart?'#444444':'#FFFFFF', border:inCart?'1.5px solid #CCCCCC':'none', opacity:busy?0.7:1 }}>
...
const css = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  .wl-card{background:#fff;border-radius:20px;overflow:hidden;border:1px solid #E0E0E0;display:flex;flex-direction:column;transition:all 0.35s}
  .wl-card:hover{transform:translateY(-7px);box-shadow:0 24px 56px rgba(0,0,0,0.1);border-color:#555555}
  ...
  .wl-view-btn{font-size:10px;font-weight:600;letter-spacing:1px;text-transform:uppercase;border:1.5px solid #E0E0E0;border-radius:8px;padding:10px 14px;cursor:pointer;transition:all 0.2s;background:#fff;color:#000000;white-space:nowrap}
  .wl-view-btn:hover{border-color:#555555;background:#F5F5F5}
  ...
  .empty-pulse{position:absolute;width:80px;height:80px;border-radius:50%;border:2px solid rgba(0,0,0,0.15);animation:pulse 2.4s ease-in-out infinite}
  @keyframes pulse{0%,100%{transform:scale(1);opacity:0.5}50%{transform:scale(1.35);opacity:0}}

  .umenu{position:absolute;top:calc(100% + 10px);right:0;background:#fff;border:1px solid #E0E0E0;border-radius:14px;padding:8px;min-width:220px;box-shadow:0 24px 60px rgba(0,0,0,0.12);animation:mIn 0.18s ease;z-index:200}
  @keyframes mIn{from{opacity:0;transform:translateY(-8px)}to{opacity:1;transform:translateY(0)}}
  .mitem{font-family:'Jost',sans-serif;font-size:13px;font-weight:500;padding:9px 12px;border-radius:9px;cursor:pointer;color:#000000;transition:background 0.15s;display:flex;align-items:center;gap:10px}
  .mitem:hover{background:#F5F5F5}
  .mitem.danger{color:#555555}
  .mitem.danger:hover{background:#F0F0F0}
