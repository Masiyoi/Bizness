...
// ── About-page icon assets ─────────────────────────────────────────
import authenticityIcon    from '../../assets/about/authenticity.png';
import speedIcon           from '../../assets/about/speed.png';
import curationIcon        from '../../assets/about/curation.png';
import communityIcon       from '../../assets/about/community.png';
import customerSuccessIcon from '../../assets/about/customer-success.png';
import coverageIcon        from '../../assets/about/coverae.png';
import ratingIcon          from '../../assets/about/rating.png';
...
import processingIcon      from '../../assets/about/processing.png';
import cashOnDeliveryIcon  from '../../assets/about/cash-on-delivery.png';
...
function InfoGrid({ items }: { items: { icon: string; label: string; value: React.ReactNode }[] }) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 12 }}>
      {items.map(it => (
        <div key={it.label} style={{ background: '#fff', borderRadius: 10, padding: '14px 16px' }}>
          {it.icon.includes('.') ? (
            <img src={it.icon} alt={it.label} style={{ width: 28, height: 28, objectFit: 'contain', marginBottom: 8, display: 'block' }} />
          ) : (
  ...
  return (
    <div style={{ overflowX: 'auto', marginBottom: 8 }}>
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
        ...
        <tbody>
          {rows.map((row, i) => (
            <tr key={i} style={{ background: '#fff', borderBottom: '1px solid rgba(0,0,0,0.12)' }}>
              {row.map((cell, j) => <td key={j} style={{ padding: '10px 14px', color: '#000', fontWeight: j === 0 ? 700 : 400 }}>{cell}</td>)}
            </tr>
          ))}
        </tbody>
      </table>
  ...
  return (
    <>
      ...
      <Section title="By the Numbers">
        <InfoGrid items={[
          { icon: productsIcon,      label: 'Products',   value: '500+ items curated'      },
          { icon: customerSuccessIcon, label: 'Customers',  value: '10,000+ happy shoppers'  },
          { icon: coverageIcon,      label: 'Coverage',   value: 'Nationwide in Kenya'     },
          { icon: ratingIcon,        label: 'Rating',     value: '4.8 / 5 avg. review'    },
        ]} />
      </Section>
...
function TabPress() {
  return (
    <>
      ...
      <AccentCard style={{ background: '#fff', color: '#000' }}><strong style={{ color: '#000' }}>Partnership opportunities:</strong> We're open to collaborations with influencers, stylists, photographers, and media outlets. Reach out and let's create something great together.</AccentCard>
    </>
  );
}
  ...
  return (
    <>
      <Section title="Our Location">
        ...
        <a href="https://www.google.com/maps/search/?api=1&query=Imenti+House+Nairobi+CBD+Kenya" target="_blank" rel="noopener noreferrer" style={{ color: '#000', fontWeight: 700, fontSize: 12, textDecoration: 'underline' }}>
          Open in Google Maps →
        </a>
      </Section>
      <AccentCard style={{ background: '#fff', color: '#000' }}>Prefer to shop from home? Our entire catalogue is available online with same-day delivery across Nairobi. <strong style={{ color: '#000' }}>WhatsApp us</strong> to arrange a viewing at our Nairobi CBD location.</AccentCard>
    </>
  );
}
...
function TabOrders() {
  const STEPS = [
    { icon: orderingIcon,      label:'Order Placed',     desc:'We receive your order and begin processing.'         },
    ...
    <>
      ...
      <Section title="Your Order Journey">
        <div style={{ position: 'relative', paddingLeft: 24 }}>
          <div style={{ position: 'absolute', left: 10, top: 12, bottom: 12, width: 2, background: 'rgba(0,0,0,0.15)', borderRadius: 2 }} />
          {STEPS.map(step => (
      ...
      <AccentCard style={{ background: '#fff', color: '#000' }}><strong style={{ color: '#000' }}>Tip:</strong> Most Nairobi CBD orders are delivered same-day or next-day. You'll always receive an SMS when your rider is on the way.</AccentCard>
    </>
  );
}
...
export default function About() {
  ...
  return (
    <div style={{ background: '#fff', minHeight: '100vh', fontFamily: "'DM Sans',system-ui,sans-serif" }}>
      <style>{`
