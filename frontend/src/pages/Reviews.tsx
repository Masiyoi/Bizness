      ...
      <div style={{ background:T.white, borderRadius:20, width:'100%', maxWidth:480, boxShadow:'0 32px 80px rgba(13,27,62,0.3)', overflow:'hidden', animation:'modalIn 0.22s ease' }}>
        ...
        <div style={{ background:T.navy, padding:'20px 24px', display:'flex', alignItems:'center', gap:14 }}>
          ...
          <div style={{ flex:1, minWidth:0 }}>
            <div style={{ fontFamily:"'Playfair Display',serif", fontWeight:700, fontSize:15, color:T.white, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{review.product_name}</div>
            <div className="jost" style={{ fontSize:10, color:`rgba(200,169,81,0.7)`, letterSpacing:'1.5px', textTransform:'uppercase', marginTop:3 }}>
              {review.product_category || 'Fashion'}
            </div>
          </div>
        ...
        <div style={{ padding:24 }}>
          ...
          <div style={{ display:'flex', gap:10 }}>
            <button onClick={onClose} className="jost"
              style={{ flex:1, background:T.creamMid, color:T.navy, border:'none', borderRadius:10, padding:'11px 0', fontSize:11, fontWeight:700, letterSpacing:'1.5px', textTransform:'uppercase', cursor:'pointer' }}>
              Cancel
            </button>
...
export default function ReviewPage() {
  ...
  const fetchReviews = useCallback(async () => {
    ...
    if (!token) return;
    try {
      const { data } = await axios.get('/api/reviews/my', { headers:{ Authorization:`Bearer ${token}` } });
      setReviews(data);
  ...
  const handleSave = async (id: number, rating: number, comment: string) => {
    const token = localStorage.getItem('token');
    const { data } = await axios.patch(`/api/reviews/${id}`, { rating, comment }, { headers:{ Authorization:`Bearer ${token}` } });
    setReviews(prev => prev.map(r => r.id === id ? { ...r, rating: data.rating, comment: data.comment, updated_at: data.updated_at } : r));
  ...
  return (
    <div style={{ fontFamily:"'Playfair Display','Georgia',serif", background:T.cream, minHeight:'100vh', color:T.text }}>
      ...
      {/* Edit Modal */}
      {editTarget && (
        <ReviewModal review={editTarget} onClose={() => setEditTarget(null)} onSave={handleSave}/>
      )}
      ...
      <div style={{ background:T.navy, padding:'40px 5% 48px', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ maxWidth:900, margin:'0 auto', position:'relative' }}>
          ...
          <p className="jost" style={{ fontSize:13, color:'rgba(255,255,255,0.45)', fontWeight:300, maxWidth:500, lineHeight:1.8 }}>
            Your honest feedback shapes the Luku Prime community. Edit or revisit any review you've left.
          </p>

          {/* Stats strip */}
          {reviews.length > 0 && (
            <div style={{ display:'flex', gap:clamp(16,20), marginTop:28, flexWrap:'wrap' }}>
              {[
      ...
      {/* ── Filter bar ── */}
      <div style={{ background:T.white, borderBottom:`1px solid ${T.creamDeep}`, padding:'14px 5%' }}>
        <div style={{ maxWidth:900, margin:'0 auto', display:'flex', gap:8, alignItems:'center', flexWrap:'wrap' }}>
          ...
          {(['all', 5, 4, 3, 2, 1] as (number|'all')[]).map(f => (
            <button key={f} onClick={() => setFilter(f)} className="jost"
              style={{ background:filter===f ? T.navy : T.cream, color:filter===f ? T.gold : T.navy, border:`1.5px solid ${filter===f ? T.navyLight : T.creamDeep}`, borderRadius:50, padding:'5px 14px', fontSize:11, fontWeight:700, cursor:'pointer', transition:'all 0.18s', letterSpacing:'0.5px' }}>
      ...
      <div style={{ maxWidth:900, margin:'0 auto', padding:'32px 5% 80px' }}>
        ...
        {loading ? (
          ...
          <div style={{ textAlign:'center', padding:'70px 0' }}>
            ...
            <div style={{ fontFamily:"'Playfair Display',serif", fontWeight:700, fontSize:20, color:T.navy, marginBottom:8 }}>
              {filter !== 'all' ? `No ${filter}-star reviews yet` : 'No Reviews Yet'}
            </div>
            <p className="jost" style={{ fontSize:13, color:T.muted, lineHeight:1.8, marginBottom:24, maxWidth:360, margin:'0 auto 24px' }}>
              {filter !== 'all'
                ...
                : 'Once you receive an order, you can leave a review on the product page.'
              }
            </p>
              ...
              <div key={review.id} className="rcard">
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:8, flexWrap:'wrap', marginBottom:6 }}>
                    <div>
                      <Link to={`/product/${review.product_id}`}>
                        <div style={{ fontFamily:"'Playfair Display',serif", fontWeight:700, fontSize:15, color:T.navy, marginBottom:4, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', maxWidth:'clamp(160px,40vw,360px)' }}>{review.product_name}</div>
                      </Link>
                    ...
                    <div style={{ display:'flex', gap:8 }}>
                      <button onClick={() => setEditTarget(review)} className="jost rcard-btn"
                        style={{ background:T.creamMid, color:T.navy, border:`1px solid ${T.creamDeep}` }}>
                        ✎ Edit
                      </button>
...
// tiny helper for inline clamp fallback
const clamp = (a: number, b: number) => b;
...
const css = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  .rcard{
    ...
    border:1px solid #E4D9C0;display:flex;gap:16px;align-items:flex-start;
  ...
  .rcard-btn{
    ...
    border-radius:6px;padding:6px 12px;cursor:pointer;transition:all 0.18s;white-space:nowrap;
  }
