...
export default function Navbar({
  ...
  const go = (path: string) => { navigate(path); setShowMenu(false); setMobileMenuOpen(false); };  const goCategory = (gender: 'men' | 'women', department: 'footwear' | 'clothing', slug: string) => {
    onCategorySelect?.(slug);
    navigate(`/shop/${gender}/${department}/${slug}`);
        ...
        <div className="nav-grid" style={{ padding: '0 5%', height: 64, display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) auto minmax(0, 1fr)', alignItems: 'center', position: 'relative' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <div className="hidden md:flex items-center gap-8">
              ...
              <div ref={saleMenuRef} style={{ position: 'relative' }}>
                ...
                {showSaleMenu && (
                  <div style={{ position: 'absolute', top: 'calc(100% + 16px)', left: '50%', transform: 'translateX(-50%)', background: '#fff', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 12, padding: 14, minWidth: 260, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', zIndex: 200, animation: 'fadeInDown 0.15s ease' }}>
                    <div style={{ fontFamily: "'Jost', sans-serif", fontSize: 9, fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase', color: '#EF4444', marginBottom: 10 }}><img src="/sale.png" alt="Sale" style={{ width: 20, height: 20, objectFit: 'contain', verticalAlign: 'middle', marginRight: 6 }} />Flash Sale</div>
              ...
              {/* ── Shop — Men/Women mega-menu with Footwear/Clothing columns ── */}
              <div ref={shopMenuRef} style={{ position: 'relative' }}>
                ...
                {showShopMenu && (
                  <div style={{ position: 'absolute', top: 'calc(100% + 16px)', left: '50%', transform: 'translateX(-50%)', background: '#fff', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 12, padding: 16, minWidth: 420, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', zIndex: 200, animation: 'fadeInDown 0.15s ease' }}>
                    <div style={{ display: 'flex', gap: 6, marginBottom: 14 }}>
                      {(['men', 'women'] as const).map(g => (
                        <button key={g} onClick={() => setShopGenderTab(g)} style={{ flex: 1, fontFamily: "'Jost', sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: '1.5px', textTransform: 'uppercase', padding: '8px 0', border: 'none', borderRadius: 8, cursor: 'pointer', background: shopGenderTab === g ? '#111' : '#f5f5f5', color: shopGenderTab === g ? '#fff' : '#666' }}>
                          {g === 'men' ? 'Men' : 'Women'}
                        </button>
                      ))}
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
                      <div>
                        <div style={{ fontFamily: "'Jost', sans-serif", fontSize: 9, fontWeight: 700, letterSpacing: '1.5px', textTransform: 'uppercase', color: 'rgba(0,0,0,0.35)', marginBottom: 6, padding: '0 14px' }}>Footwear</div>
                        {(categoryTree?.[shopGenderTab]?.footwear ?? []).map(c => (
                          <button key={c.id} className="mitem" onClick={() => goCategory(shopGenderTab, 'footwear', c.slug)}>{c.name}</button>
                        ))}
                      </div>
                      <div>
                        <div style={{ fontFamily: "'Jost', sans-serif", fontSize: 9, fontWeight: 700, letterSpacing: '1.5px', textTransform: 'uppercase', color: 'rgba(0,0,0,0.35)', marginBottom: 6, padding: '0 14px' }}>Clothing</div>
                        {(categoryTree?.[shopGenderTab]?.clothing ?? []).map(c => (
                          <button key={c.id} className="mitem" onClick={() => goCategory(shopGenderTab, 'clothing', c.slug)}>{c.name}</button>
                        ))}
                      </div>
                    </div>
                    <div style={{ margin: '10px 0', borderTop: '1px solid rgba(0,0,0,0.06)' }} />
                    <button className="mitem" style={{ justifyContent: 'center', fontWeight: 700 }} onClick={() => { navigate(`/shop/${shopGenderTab}`); setShowShopMenu(false); }}>
                      Shop All {shopGenderTab === 'men' ? 'Men' : 'Women'} →
                    </button>
                  </div>
                )}
              </div>

              <span className="nav-link" style={{ color: ink }} onClick={() => navigate('/pw-essentials')}>PW Essentials</span>
              {/* ── Members Club — last ── */}
      ...
      {mobileMenuOpen && (
        <div style={{ background: 'rgba(255,255,255,0.28)', backdropFilter: 'blur(20px) saturate(180%)', WebkitBackdropFilter: 'blur(20px) saturate(180%)' as any, borderBottom: '1px solid rgba(255,255,255,0.25)', padding: '16px 5% 28px', position: 'fixed', top: 96, left: 0, right: 0, bottom: 0, zIndex: 99, transition: 'top 0.3s ease', overflowY: 'auto', overscrollBehavior: 'contain', WebkitOverflowScrolling: 'touch' as any }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 16 }}>
            ...
            <button
              onClick={() => { navigate('/pw-essentials'); setMobileMenuOpen(false); }}
              style={{
                fontFamily: "'Jost', sans-serif", fontSize: 10, fontWeight: 600,
                letterSpacing: '2px', textTransform: 'uppercase',
          ...
          <div style={{ borderBottom: '1px solid rgba(0,0,0,0.05)', padding: '11px 14px 14px' }}>
            <button
              ...
              onClick={() => go('/sale')}
            >
              <img src="/sale.png" alt="Sale" style={{ width: 22, height: 22, objectFit: 'contain', verticalAlign: 'middle', marginRight: 6 }} />Sale
            </button>
            {flashProducts.length > 0 && (
              <div style={{ display: 'flex', gap: 8 }}>
                {flashProducts.map((p: any) => (
          ...
          </div>          {/* ── Shop — Men/Women tabs with Footwear/Clothing subcategories ── */}
          <div className="mitem" style={{ borderRadius: 0, padding: '11px 14px', fontWeight: 700, cursor: 'default' }}>
            Shop
          </div>
          <div style={{ borderBottom: '1px solid rgba(0,0,0,0.05)', paddingBottom: 8 }}>
