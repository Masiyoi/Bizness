      ...
      <div className="max-w-content mx-auto">
        ...
        {loading && (
          <div className="grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-4">
            {[...Array(6)].map((_, i) => (
              <div key={i} className="bg-white rounded-[16px] overflow-hidden border border-cream-deep">
        ...
        {!loading && reviews.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="font-sans text-[14px] text-muted">No reviews yet — be the first after your purchase!</p>
          </div>
        )}
            ...
            {reviews.map((review, idx) => {
              ...
              return (
                <div
                  ...
                  className="group bg-white rounded-[16px] overflow-hidden border-[1.5px] border-cream-deep cursor-pointer transition-all duration-200 hover:border-gold hover:-translate-y-[3px] flex flex-col relative"
                  style={{ animationDelay: `${idx * 0.05}s` }}
                >
                  {/* ── Product strip (navy header) ── */}
                  <div className="flex items-center gap-2.5 px-3.5 py-2.5 bg-navy">
                    <img
                      src={review.image_url ?? `https://placehold.co/40x40/152348/C8A951?text=LP`}
                      alt={review.product_name}
                      onError={e => { (e.target as HTMLImageElement).src = `https://placehold.co/40x40/152348/C8A951?text=LP`; }}
                      className="w-10 h-10 rounded-lg object-cover shrink-0 border-2 border-gold/30"
                    />
                    <div className="flex-1 min-w-0">
                      <div className="font-sans text-[11px] font-bold text-white overflow-hidden text-ellipsis whitespace-nowrap">
                        {review.product_name}
                      </div>
                      {review.category && (
                    ...
                    {/* Appears on hover — signals clickability */}
                    <span className="font-sans text-[10px] font-bold text-gold tracking-[1px] opacity-0 group-hover:opacity-100 transition-opacity duration-200 shrink-0">
                      View →
                    </span>
                  </div>
                  ...
                  <div className="p-4 flex-1 flex flex-col gap-2.5">
                    ...
                    <div className="absolute top-[52px] right-3 font-serif text-[48px] leading-none text-gold/10 select-none pointer-events-none">
                      "
                    </div>
                    ...
                    <div className="flex justify-between items-center">
                      <div className="flex gap-0.5">
                        {[1,2,3,4,5].map(s => (
                          <span key={s} style={{ fontSize:12, color: s <= review.rating ? T.gold : T.creamDeep }}>★</span>
                        ))}
                      </div>
                    ...
                    <div className="flex items-center gap-2.5 pt-2.5 border-t border-cream-deep mt-auto">
                      <div
                        className="w-[34px] h-[34px] rounded-full flex items-center justify-center shrink-0 border-2 border-cream-deep"
                        style={{ background: avatarBg }}
                      >
                        <span className="font-sans text-[11px] font-extrabold text-white tracking-[0.5px]">
                          {initials}
                        </span>
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        <span className="inline-flex items-center gap-1 font-sans text-[9px] font-bold text-[#4A7A4A] bg-[#EEF3EE] border border-[#C8DFC8] rounded-full px-2 py-0.5 mt-0.5">
                          ✓ Verified purchase
                        </span>
                      </div>
                    </div>
